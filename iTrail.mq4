...
//      ticket = OrderSend(Symbol(), OP_BUY, lotSize, Ask, 0, Bid - stopLoss, 0);
      ticket = OrderSend(Symbol(), OP_BUY, lotSize, Ask, 0, 0, Bid + stopLoss);
    }
    else if(position == OP_SELL) {
//      ticket = OrderSend(Symbol(), OP_SELL, lotSize, Bid, 0, Ask + stopLoss, 0); 
      ticket = OrderSend(Symbol(), OP_SELL, lotSize, Bid, 0, 0, Ask - stopLoss); 
    }
    else {
...
//      if(OrderStopLoss() < Bid - stopLoss) {
      if(Bid + stopLoss < OrderTakeProfit()) {
        bool modified = OrderModify(ticket, OrderOpenPrice(), 0, Bid + stopLoss, 0);
      }
    }
    else if(OrderType() == OP_SELL) {
//      if(Ask + stopLoss < OrderStopLoss()) {
      if(OrderTakeProfit() < Ask - stopLoss) {
        bool modified = OrderModify(ticket, OrderOpenPrice(), 0, Ask - stopLoss, 0);
      }                     
    }
    else {
